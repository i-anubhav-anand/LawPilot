...
class VectorStore:
    ...
    async def add_document(
        ...
        # Ensure the session_id is properly tracked in metadata 
        if "session_id" in metadata and metadata["session_id"]:
            session_id = metadata["session_id"]
            logger.info(f"📄 DOCUMENT ASSOCIATED WITH SESSION: doc_id={document_id}, session_id={session_id}")
            
            # Initialize the session documents tracking if needed
            if not hasattr(self, 'session_documents'):
                self.session_documents = {}
                
            # Add this document to the session's documents
            ...
            if document_id not in self.session_documents[session_id]:
                ...
                logger.info(f"📄 ADDED DOCUMENT TO SESSION TRACKING: doc_id={document_id}, session_id={session_id}")
        ...
        try:
            ...
            for idx, (index, distance, similarity) in enumerate(zip(indices, distances, similarities)):
                ...
                if filter_metadata:
                    ...
                    # Debug logging for metadata filtering
                    logger.debug(f"METADATA FILTER: {filter_metadata}, DOCUMENT METADATA: {doc_metadata}")
                    
                    for key, value in filter_metadata.items():
                        # Check if key exists in metadata
                        if key not in doc_metadata:
                            logger.debug(f"KEY {key} NOT FOUND IN METADATA FOR DOC {doc_id}")
                            metadata_match = False
                            break
                            
                        # For session_id, we need special handling to ensure correct matching 
                        if key == "session_id" and value != doc_metadata.get(key):
                            logger.debug(f"SESSION_ID MISMATCH: filter={value}, doc={doc_metadata.get(key)}")
                            metadata_match = False
                            break
                            
                        # For other metadata, just do a direct comparison
                        if key != "session_id" and doc_metadata.get(key) != value:
                            logger.debug(f"METADATA MISMATCH: key={key}, filter={value}, doc={doc_metadata.get(key)}")
                            metadata_match = False
                            break
