...
async def process_chat(
    ...
    """
    Process a chat message using the RAG engine.
    
    This endpoint handles processing the chat message, updating the case file with
    any extracted facts, and returning sources with the response.
    
    Args:
        chat_request: Chat request with query, session_id and optionally case_file_id
        rag_engine: RAG engine dependency
        case_file_service: Case file service dependency
        background_tasks: Background tasks dependency for async processing
        
    Returns:
        A ChatResponse with the answer and any sources
    """
    logger.info(f"🔄 RECEIVED CHAT REQUEST: query={chat_request.query}, session_id={chat_request.session_id}")
    ...
    try:
        ...
        logger.info(f"🔄 PROCESSING QUERY WITH RAG ENGINE")
        rag_response = await rag_engine.process_query(
            query=chat_request.query,
            session_id=chat_request.session_id,
            case_file=case_file,
            num_results=chat_request.num_results
        )
        logger.info(f"✅ RAG QUERY PROCESSED")
        
        # If we're working with a case file, update the case summary in the background
        if case_file and background_tasks:
            # Schedule background update of case summary based on chat history
            async def update_summary_background():
                ...
                logger.info(f"🔄 UPDATING CASE SUMMARY IN BACKGROUND: case_file_id={chat_request.case_file_id}")
                chat_history = rag_engine.get_chat_history(chat_request.session_id)
                
                # Convert to dict format for the summary generator
                chat_history_dicts = [
                ...
                # Update the summary
                summary_manager = CaseSummaryManager()
                await summary_manager.generate_summary_from_chat(
                    case_file_id=chat_request.case_file_id,
                    session_id=chat_request.session_id,
                    chat_history=chat_history_dicts
                )
                logger.info(f"✅ CASE SUMMARY UPDATED: case_file_id={chat_request.case_file_id}")
        ...
        logger.info(f"✅ CHAT RESPONSE READY: session_id={chat_request.session_id}")
        return response
...
async def process_chat_message(
    ...
    """
    Alternative endpoint to process chat messages (compatible with frontend calls).
    """
    logger.info(f"🔄 RECEIVED CHAT REQUEST (/message ENDPOINT): session_id={chat_request.session_id}")
    return await process_chat(chat_request, rag_engine, case_file_service, background_tasks)
...
async def list_chat_sessions(
    ...
    session_ids = rag_engine.list_chat_sessions()
    
    sessions = []
    for session_id in session_ids:
        # Get the chat history to calculate message count
        history = rag_engine.get_chat_history(session_id)
        message_count = len(history)
        
        # Try to get creation time from the first message, if available
        created_at = None
        if history and hasattr(history[0], 'timestamp'):
            created_at = history[0].timestamp.isoformat()
